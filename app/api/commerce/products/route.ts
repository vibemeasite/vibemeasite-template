...
// 20) — same reasoning as app/api/booking/widget-config/route.ts. No
// cookie involved (product listing is public), unlike the cart/checkout
// relays below.
...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${PUBLIC_COMMERCE_BASE}products?${params.toString()}`);
